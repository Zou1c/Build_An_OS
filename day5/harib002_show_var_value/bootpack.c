...
void HariMain(void)
{
	...
	putfonts8_ascii_str(binfo->vram, binfo->scrnx, 16, 64, COL8_FFFFFF, s);
	char s_[40];
	sprintf(s_, "&scrnx = %x (format:0x)", &(binfo->scrnx));
	putfonts8_ascii_str(binfo->vram, xsize, 16, 80, COL8_FFFFFF, s_);


	char s_s[40];
	sprintf(s_s, "scrny = %d", binfo->scrny);
	putfonts8_ascii_str(binfo->vram, xsize, 128, 64, COL8_FFFFFF, s_s);
	char s_s_[40];
	sprintf(s_s_, "&scrny = %x (format:0x)", &(binfo->scrny));
	putfonts8_ascii_str(binfo->vram, xsize, 128, 40, COL8_FFFFFF, s_s_);
